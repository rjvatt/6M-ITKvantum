...
>Код для решения всех 7 задач отправить письмом на почту pyatkevich.vladislav@yandex.ru  
...
### Длина строки
Для подсчета количества символов в строке (включая пробелы) есть удобная команда **len()** (сокращение от английского слова length - "длина"). По сути, **len()** - это так называемая *функция* (ты будет часто пользоваться функциями, работая над проектами). Чтобы узнать длину строки "Инопланетянин", необходимо поместить строку внутрь функции len("Инопланетянин"), и вывести это значение на экран с помощью функции print(), например:  
~~~python
string = 'инопланетянин Ино' # Не забывай про кавычки
length = len(string) # В переменную length поместили длину строки message
print(length) # Выводит на экран значение переменной length, то есть длину строки "инопланетянин Ино"
~~~
Вычислить длину строки можно немного короче:  
~~~python
...
print(c)
~~~
3. Чему равен результат умножения строки "Hi" на число 10? 

>Код для решения всех задач отправить письмом на почту pyatkevich.vladislav@yandex.ru  
В теме письма указать "Домашнее задание Иванов", где Иванов заменить на свою фамилию. 

## Тема №3: Python. Списки.
Сохранить сразу несколько данных (особенно если важен их порядок) поможет список. Каждому его элементу Python присваивает номер, означающий позицию в списке. Содержимое списка можно изменять как угод и когда угодно.  
...
~~~python
rockets_players = ['Рома', 'Игорь', 'Алёна', 'Рената', 'Саша', 'Лена'] # Элементы списка нужно разделять запятыми
planets_players = ['Миша', 'Павел', 'Полина', 'Настя', 'Аня', 'Лёша']
~~~
...
~~~python
print(rockets_players[0]) # Выведет "Рома"
print(rockets_players[5]) # Выведет "Лёша"
~~~

### Домашнее задание №3
**Вычислить с помощью Python:**  
Скопируйте данный код и вставьте в программы: 
~~~python
rockets_players = ['Рома', 'Игорь', 'Алёна', 'Рената', 'Саша', 'Лена'] 
planets_players = ['Миша', 'Павел', 'Полина', 'Настя', 'Аня', 'Лёша']
~~~

***Весь код для выполнения домашнего задания вставляйте после скопированных строк!***

**Задачи:**

1. Что выведет на экран следующий код?
~~~python
print(rockets_players[2])
print(planets_players[3])
~~~

2. Выведете на экран четвертый по счёту элемент списка rockets_players

3. Выведете на экран второй по счёту элемент списка planets_players
